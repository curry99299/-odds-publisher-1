...
每場兩隊：winnerteam=贏家、secondteam=輸家（class 為輸贏高亮，非主客固定位置；贏家可能排上或排下）。
比分要靠 class 對齊：winnerteam 的分數是 <li class="winnerscores">，secondteam 的分數是另一個數字 li，
不能靠 li 出現順序取值（winnerscores 的位置會變，靠順序會把兩隊分數對調）。
